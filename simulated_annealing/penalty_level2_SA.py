...
class Optimization:
    ...
    def objective_function(self, new_path, travel_time_matrix, service_time_vector, 
        ...
        print("bool_capacity_result:", bool_capacity_result)
        print("payload_vector:", payload_vector)
        if bool_capacity_result:
            recharge_time_vector = self.check_refueling_constraints(new_path, 
                                                         payload_vector, 
                                                         travel_time_matrix, 
                                                         max_energy, power_function)
        else:
            recharge_time_vector = np.zeros((1,len(new_path)))
        ...
        return total_travel_time + additional_time + PENALTY_CONST * (1 - bool_capacity_result)
...
#         print("type paths visited:", type(self.paths_visited_history))
    ...
    def simulated_annealing(self, n_iterations, temperature, initial_guess):
        ...
        print("initial guess:", current_path)
        ...
        for i in range(n_iterations):
            ...
            STOP_N = 500
            if (len(self.objective_value_history)>= STOP_N) and \
                (round(sum(self.objective_value_history[-STOP_N:])/STOP_N, 2) == round(self.objective_value_history[-1], 2)):
                return current_path
